...
/*====================================================================================================*/
/*====================================================================================================*
**函数原型 : uint8_t ICM_20689_Init(void)
**功    能 : ICM_20689函数初始化
**输    入 : None
**输    出 : None
**备    注 : 
...
{ 
	  ICM_20689_WriteRegister(ICM20689_RA_PWR_MGMT_1,0X80);	//复位icm20689
	  delay_ms(100);
	  ICM_20689_WriteRegister(ICM20689_RA_SIGNAL_PATH_RESET,ICM20689_BIT_GYRO | ICM20689_BIT_ACC | ICM20689_BIT_TEMP); //Reset accel digital signal path. Reset temp digital signal path.
	  delay_ms(100);
	  ICM_20689_WriteRegister(ICM20689_RA_PWR_MGMT_1,0X00); //唤醒icm20689

    ICM_20689_WriteRegister(ICM20689_RA_USER_CTRL, 0x10); //USER控制模式
    delay_us(50);

    ICM_20689_WriteRegister(ICM20689_RA_PWR_MGMT_2, 0x00); //电源2管理,全部打开读写
    delay_us(50);

    //陀螺仪采样率0x00(1000Hz)   采样率 = 陀螺仪的输出率/ (1 + SMPLRT_DIV)
	  ICM_20689_WriteRegister(ICM20689_RA_SMPLRT_DIV, 0x00); //电源2管理,全部打开读写
    delay_us(50);
    ...
    ICM_20689_WriteRegister(ICM20689_RA_INT_PIN_CFG, 0 << 7 | 0 << 6 | 0 << 5 | 0 << 4 | 0 << 3 | 0 << 2 | 1 << 1 | 0 << 0);
    delay_us(50);

    //低通滤波
    ICM_20689_WriteRegister(ICM20689_RA_CONFIG, ICM20689_LPF_42HZ);
    delay_us(50);

    //陀螺仪自检及测量范围,典型值0x18(不自检,2000deg/s) (0x10 1000deg/s) (0x10 1000deg/s) (0x08 500deg/s)
		ICM_20689_WriteRegister(ICM20689_RA_GYRO_CONFIG,0x18);
    delay_us(50);

    //加速度自检及测量范围(典型值,+-8G)
    ICM_20689_WriteRegister(ICM20689_RA_ACCEL_CONFIG,2 << 3);
    delay_us(50);

    //加速度低通滤波
    
    ICM_20689_WriteRegister(ICM20689_RA_ACCEL_CONFIG2,ICM20689_LPF_42HZ);
    delay_ms(5);
	
	return ICM_20689_SpiDetect();					//器件ID不正确,返回false
...
/*====================================================================================================*/
/*====================================================================================================*
**函数原型 : void ICM_20689_Read_Data(void)
**功    能 : 读取
**输    入 : None
**输    出 : None
**备    注 : 
...
void ICM_20689_Read_Data(void)
{
   ...
   uint8_t reg=0x80|ICM20689_RA_ACCEL_XOUT_H;
...
/*====================================================================================================*/
/*====================================================================================================*
**函数原型 : uint8_t ICM_20689_SpiDetect(void)
**功    能 : 配置写寄存器
**输    入 : None
**输    出 : None
**备    注 : 
...
uint8_t ICM_20689_SpiDetect(void)
{
	...
	ICM_20689_ReadRegister(ICM20689_RA_WHO_AM_I, 1, &tmp);
...
/*====================================================================================================*/
/*====================================================================================================*
**函数原型 : uint8_t ICM_20689_WriteRegister(uint8_t reg,uint8_t data)
**功    能 : 配置写寄存器
**输    入 : None
**输    出 : None
**备    注 : 
...
/*====================================================================================================*/
/*====================================================================================================*
**函数原型 : uint8_t ICM_20689_ReadRegister(uint8_t reg, uint8_t length, uint8_t *data)
**功    能 : 配置读寄存器
**输    入 : None
**输    出 : None
**备    注 : 
